...
<article>
  ...
  ${DEMO_PUBLIC ? `<p>If you request a live demo, we also ask for your business name, your city and state, and a mobile number. That is all we ask for. We use the mobile number for two things and nothing else: so that Jenny recognizes your number when you call the demo line, and to send you a one time code confirming the number is yours. Those are the only messages we send to it. We do not use it for marketing texts. You can reply STOP to any message to stop them, and message and data rates may apply.</p>
  <p>If you run a live demo, that phone call is recorded so that we can email you the recording. We keep the recording only as long as it is needed for that purpose, and we delete it on your request, the same as the other information you give us.</p>
